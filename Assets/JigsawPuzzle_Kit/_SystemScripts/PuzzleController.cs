...
public class PuzzleController : MonoBehaviour
{
        ...
        if (changeOnlyRotation)
        {
            ...
            for (int i = 0; i < pieces.Length; i++)
                if (_filterList == null || (_filterList != null && _filterList.Contains(i)))
                {
                    ...
                    switch (decomposeSides[randomDecomposeSide] + 1)
                    {
                        //case 0: // Left
    ...
    public void AlignWithCameraCenter(Camera _camera, bool changeCameraSize = true, float _cameraSizeTuner = 0)
    {
        ...
        if (_camera)
        {
            ...
            transform.position = new Vector3(screenviewCenter.x - puzzleBounds.size.x / 2, screenviewCenter.y + puzzleBounds.size.y / 2 + 1.5f, transform.position.z);
            Prepare();
